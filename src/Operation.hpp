...
class Operation{
    public:
        Operation();
        Operation(std::string n, Resource_Type t, bool sn, int width, int id, int out=-1, int a0=-1, int a1=-1, int a2=-1, std::string sym="NOP");

        int id;                                     //The ID of the node, used for graph generation
        int datawidth = 1;                          //Datawith, used for verilog
        std::string name = "default";                    //Variable name (for in/out/reg/wire), used for verilog
        Resource_Type type = Resource_Type::NOP;    //Type, used for graph generation 
        bool sign = false;                          //sign, used for verilog
        std::string symbol = "none";                     //the symbol of the operation (i.e "+" or "<<"), used for verilog
